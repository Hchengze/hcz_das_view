...
def make_fk_filter_zd_file(tmp_path):
    path = tmp_path / "fk_filter_service.h5"
...
def test_compute_fk_filter_for_file_with_synthetic_zd_hdf5(tmp_path):
    path = make_fk_filter_zd_file(tmp_path)
    ...
    assert service_result.das_data.data.shape == (128, 16)
    assert service_result.result.mask.shape == (
        service_result.result.frequencies_hz.size,
        service_result.result.wavenumbers_cycles_per_m.size,
    )
    assert service_result.result.filtered_fk is not None
    assert service_result.filter_parameters["vmin_mps"] == 100.0
    assert service_result.filter_parameters["vmax_mps"] == 2000.0


def test_compute_fk_filter_for_file_applies_bandpass_step(tmp_path):
    path = make_fk_filter_zd_file(tmp_path)
    steps = [
        (
            "bandpass",
